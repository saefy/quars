...
db_type = mysql','html');
		...
		if(mysql_select_db($D)!=false){
			...
			try{
				throw new QrsException("Error al Seleccionar la db: ");
			}catch(QrsException $e){
				$e->description='Mysql Respondi&oacute;:'. mysql_error().'</b>';
	...
	public function insert($table,$array_fields,$id_field_name,$form_fields){
		...
		foreach($array_fields as $f_name=>$f_val){
			...
			if($f_name!=$id_field_name && trim($f_name)!=''){
				...
				//if($f_name=='se_fabrica'){ pa($form_fields[$f_name]); die();}
				...
				if($FieldType=='password'){
					...
					if($f_val===NULL || trim($f_val)==''){
						...
						$f_val = substr($f_val, 0,10); // Cortar date
						$fields_vals .= "CONCAT(STR_TO_DATE('".$this->escape_string($f_val)."', '".DB_DATE_FORMAT."'),' ','".$f_val_time."'),";
					}
				}elseif(($FieldType=='int' || $FieldType=='decimal' || $FieldType=='money' || $FieldType=='select' || $FieldType=='search_field' || $FieldType=='file' || $FieldType=='checkbox' 
					|| $FieldType=='tinyint' ) ){
					// Exception numeric

					if (($f_val===NULL || trim($f_val)=='') && ($form_fields[$f_name]['Null'] == 'YES')) {

						$fields_vals .= " NULL ,";
					} else {
						if ($f_val===NULL || trim($f_val)==''){
							$f_val = ($form_fields[$f_name]['Default']!='') ? $form_fields[$f_name]['Default']: 0;
